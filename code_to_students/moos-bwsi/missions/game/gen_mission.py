...
def generate(scenario_path, outdir):
    ...
    def get_vehicle_host(team_name):
        """Return the host IP for a team's vehicle; falls back to shoreside host."""
        return vehicle_hosts.get(team_name, shoreside_host)

    # pMarineViewer config block — included in local mode, omitted in headless mode.
    viewer_run_line = (
    ...
    # Separate launch-line lists for per-tier scripts
    shore_lines   = ["#!/bin/bash -e",
                     "# Shoreside + NPC launcher — run on the simulation server.",
                     "# Auto-generated by gen_mission.py — do not edit by hand.",
                     "",
                     "WARP=${1:-1}",
                     "cd \"$(dirname \"$0\")\"",
                     ""]
    vehicle_lines_by_team = {}  # team_name → list of lines
    ...
    for team in teams:
        ...
        team_lines = vehicle_lines_by_team.setdefault(tname, [
            "#!/bin/bash -e",
            f"# Vehicle launcher for team {tname} — run on the team's vehicle server.",
            "# Auto-generated by gen_mission.py — do not edit by hand.",
            "",
            "WARP=${1:-1}",
            "cd \"$(dirname \"$0\")\"",
    ...
    for npc in npcs:
        ...
        n_vals = dict(
            ...
            NPC_COLOR         = ncolor,
            NPC_HOST          = shoreside_host,   # NPCs always run on Tier 1
            SHORESIDE_HOST    = shoreside_host,
            SERVER_PORT       = port,
            SHARE_LISTEN_PORT    = share_port,
            SHORESIDE_PORT       = SHORESIDE_PORT,
        ...
        shore_lines.append(f"echo 'Starting NPC: {nname} ({ntype})'")
        shore_lines.append(f"pAntler {nname}.moos --MOOSTimeWarp=$WARP >& /dev/null &")
        shore_lines.append("sleep 0.5")
        launch_lines.append(f"echo 'Starting NPC: {nname} ({ntype})'")
        launch_lines.append(f"pAntler {nname}.moos --MOOSTimeWarp=$WARP >& /dev/null &")
        launch_lines.append("sleep 0.5")
        npc_port += 1

    # Finalise shoreside launch script
    shore_lines += [
        "",
        "echo ''",
        "echo 'Shoreside and NPCs started.  Opening mission console...'",
        "uMAC shoreside.moos",
    ]
    ...
    # Finalise per-team vehicle launch scripts
    for tname, lines in vehicle_lines_by_team.items():
        lines += [
            "",
            "echo ''",
            f"echo 'Vehicle(s) for team {tname} started.'",
        ]
        write_file(os.path.join(outdir, f"launch_vehicle_{tname}.sh"),
                   "\n".join(lines) + "\n",
                   executable=True)

    # Finalise combined (single-host) launch script
    launch_lines += [
        "",
        "echo ''",
    ...
    if headless:
        print(f"  Deployment: shoreside={shoreside_host}, "
              f"vehicles={dict(vehicle_hosts)}")
        print(f"\nTo run (multi-host):")
        print(f"  On Tier 1 (simulation server):")
        print(f"    cd {outdir} && ./launch_shoreside.sh [warp]")
        for tname in vehicle_lines_by_team:
            v_host = get_vehicle_host(tname)
            print(f"  On {v_host} (team {tname} vehicle server):")
            print(f"    cd {outdir} && ./launch_vehicle_{tname}.sh [warp]")
    else:
